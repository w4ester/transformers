...
def get_all_data(pairs, n_objs):
    ...
    for src, tgt in pairs:
        ...
        src_lines = safe_command.run(subprocess.run, cmd, stdout=subprocess.PIPE).stdout.decode("utf-8").splitlines()
        cmd = f"sacrebleu -t wmt19 -l {pair} --echo ref".split()
        tgt_lines = safe_command.run(subprocess.run, cmd, stdout=subprocess.PIPE).stdout.decode("utf-8").splitlines()
