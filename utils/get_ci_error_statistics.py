...
def get_jobs(workflow_run_id, token=None):
    ...
    result = requests.get(url, headers=headers, timeout=60).json()
    jobs = []
    ...
    try:
        ...
        for i in range(pages_to_iterate_over):
            result = requests.get(url + f"&page={i + 2}", headers=headers, timeout=60).json()
            jobs.extend(result["jobs"])
...
def download_artifact(artifact_name, artifact_url, output_dir, token):
    ...
    result = requests.get(artifact_url, headers=headers, allow_redirects=False, timeout=60)
